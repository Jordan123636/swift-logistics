...
function generateTrackingId() {
  ...
  return 'SWL-' + prefix + numbers;
}


// ── ON PAGE LOAD: Pre-fill quote form if coming back from auth ──
window.addEventListener('load', () => {
  const saved = sessionStorage.getItem('pendingQuote');
  if (!saved) return;

  const data = JSON.parse(saved);
  const fields = ['fullName', 'company', 'email', 'phone', 'origin', 'destination', 'weight', 'details'];
  fields.forEach(id => {
...
async function handleLogin() {
  ...
  try {
    ...
    if (pending) {
      ...
      document.getElementById('successSub').textContent = "You're signed in successfully.";
...
async function handleSignup() {
  ...
  if (!firstName || !lastName || !email || !password) { showError('signupError', 'Please fill in all fields.'); return; }
  if (!email.includes('@')) { showError('signupError', 'Please enter a valid email address.'); return; }
  if (password.length < 8) { showError('signupError', 'Password must be at least 8 characters.'); return; }
  if (!terms) { showError('signupError', 'Please accept the Terms of Service to continue.'); return; }
